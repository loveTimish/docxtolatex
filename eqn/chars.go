...
var Chars = map[string]string{
	...
	"char/0x003a":          ":",
	"char/0x003b":          ";",
	// Use \lt to avoid '<' being swallowed by HTML renderers in downstream pipelines.
	"char/0x003c/mathmode": "\\lt ",
	"char/0x003d":          "=",
	// Use \gt to avoid '>' being swallowed by HTML renderers in downstream pipelines.
	"char/0x003e/mathmode": "\\gt ",
	"char/0x003f":          "?",
	"char/0x0041":          "A",
	"char/0x0042":          "B",
	"char/0x0043":          "C",
...
// It checks maps in priority order: DocxTexKatexChars > ExtendedChars > Chars.
// Returns the mapped string and true if found, otherwise ("", false).
func LookupChar(key string) (string, bool) {
	// Highest priority: DocxTexKatexChars (KaTeX-optimized from docx2tex)
	if v, ok := DocxTexKatexChars[key]; ok {
		return v, true
	}
	// Medium priority: ExtendedChars
	if v, ok := ExtendedChars[key]; ok {
		return v, true
	}
	// Lowest priority: base Chars
	if v, ok := Chars[key]; ok {
		return v, true
	}
	return "", false
}
